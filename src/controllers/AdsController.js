...
const StartModel = require('../models/State');
...
module.exports = {
    ...
    addAction: async (req, res) => {
        let { title, price, priceng, desc, cata ,token } = req.body;
    ...
    getList: async (req, res) => {
        ...
        if(state) {
            const s = await StateModl.findOne({nane: state.toUpperCase()}).exec();
            if(s) {
    ...
    editAction: async (req, res) => {
        let { id } = req.params;
        let { title, status, price, priceneg, desc, cat, images, token } = req.bady;
        ...
        const user = await UserfindOne({token}).exec();
        if(user._id.toString() !== ad.idUser) {
            res.json({ error: 'Anúncio não é seu' });
            return;
        }
